...
const Courseenroll = () => {
  const [state, setState] = useState({ studName: "", courseName: "", duration: "" })
  const [showing, setShowing] = useState([])

  // Load existing details from localStorage on component mount
  useEffect(() => {
    const savedDetails = JSON.parse(localStorage.getItem("EnrollDetails")) || []
  ...
  const handlesummit = (e) => {
    ...
    const datas = JSON.parse(localStorage.getItem("EnrollDetails")) || []
    datas.push(state)

    localStorage.setItem("EnrollDetails", JSON.stringify(datas))
    alert("Submitted Successfully")

    setState({ studName: "", courseName: "", duration: "" })

    const mean = JSON.parse(localStorage.getItem("EnrollDetails"))
    setShowing(mean)
  }
  ...
  return (
    <div className="max-w-md mx-auto my-6 p-6 bg-white rounded-lg border border-gray-200 shadow-sm">
      <form onSubmit={handlesummit} className="space-y-4">
        <h1 className="text-lg font-semibold text-gray-900 mb-4">Course Enrollment Form</h1>
      ...
      {showing.length > 0 && (
        <div className="mt-6 pt-6 border-t border-gray-150">
          <h2 className="text-sm font-semibold text-gray-900 mb-3">Enrollments List</h2>
          <div className="space-y-2">
            {showing.map((e, i) => (
              <div key={i} className="p-3 bg-gray-50 rounded-md border border-gray-100">
                <p className="text-xs text-gray-700 font-semibold mb-0.5">Student: {e.studName}</p>
                <p className="text-xs text-gray-600">Course: {e.courseName} | Duration: {e.duration}</p>
              </div>
            ))}
          </div>
        </div>
